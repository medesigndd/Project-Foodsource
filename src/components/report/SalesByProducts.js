...
let config = {
    ...
    tooltip: {
        //pointFormat: '{series.name}:<b>{point.percentage:.1f}%</b><br/>จำนวนเงิน {point.y} บาท'
...
class SalesByProducts extends React.Component {
    ...
    componentWillReceiveProps(nextProps, nextState) {
        this.updateStyle(nextProps.docked, nextProps.width, nextProps.height, nextProps.salesByProducts);
    }

    componentWillMount() {
        this.updateStyle(this.props.docked, this.props.width, this.props.height, this.props.salesByProducts);
        this.props.fetchSalesByProducts();
    }
    ...
    search = () => {
        this.props.fetchSalesByProducts($("#startDate").val(), $("#endDate").val());
    }
    ...
    render() {
        ...
        if (this.props.displayLoading) {
            ...
            display = <div>{/* style={{ background: 'transparent url(./images/bg.png) repeat 0 0', }} */}
                <ReactHighcharts config={config} options={options} />
            </div>
        }
        return (
...
function mapStateToProps(state) {
    return {
        ...
        salesByProducts: state.analytic.salesByProducts,
    }
}

export default connect(mapStateToProps, actions)(SalesByProducts);